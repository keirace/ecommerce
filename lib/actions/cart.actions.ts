...
export const addProductToCart = async (selectedVariant: Variant | null, setIsCartOpen: React.Dispatch<React.SetStateAction<boolean>>) => {
    ...
    // fetch guest id
    const guestId = await db.select({id: schema.guests.id}).from(schema.guests).where(eq(schema.guests.sessionToken, data.sessionToken)).limit(1).then(res => res[0]?.id || null);
